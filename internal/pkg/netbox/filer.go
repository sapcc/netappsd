...
func (c Client) GetFilers(ctx context.Context, region, query string) (filers []Filer, err error) {
	switch query {
	case "md", "manila":
		filers, err = c.getFilers(ctx, region, "manila")
	case "bb", "cinder":
		filers, err = c.getFilers(ctx, region, "cinder")
	case "bm", "baremetal":
		filers, err = c.getFilers(ctx, region, "baremetal")
	case "apod", "cp", "control-plane", "control_plane":
		filers, err = c.getFilers(ctx, region, "apod")
	default:
